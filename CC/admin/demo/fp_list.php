...
<body class="body">
    ...
    <tbody>
    <?php 
        $query="SELECT * FROM film_platoon order by fp_id";
        $get = mysqli_query($con,$query);
            //统计要显示的记录总数
        $rec_count = mysqli_num_rows($get);
            //分页显示符合要求的记录
        $sql1="SELECT * FROM film_platoon order by fp_id LIMIT $offset,$page_size";
        $get = mysqli_query($con,$sql1);
   
        while($row=mysqli_fetch_assoc($get)){
            echo"<tr><td>".$row["fp_id"].
            "</td><td>".$row["m_name"].
            "</td><td>".$row["hall"].
            "</td><td>".$row["film_date"].
            "</td><td>".$row["start_time"].
            "</td><td>".$row["over_time"].
            "</td><td><a href='up_fp.php?fp_id={$row["fp_id"]}'>"."修改"."</a></td>". 
            "</tr>";
        }
        ?>   
