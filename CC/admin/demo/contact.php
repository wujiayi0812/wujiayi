...
<body class="body">
    ...
    <tbody>
        ...
        $query="SELECT * FROM contact order by con_id";
        $get = mysqli_query($con,$query);
            //统计要显示的记录总数
        $rec_count = mysqli_num_rows($get);
            //分页显示符合要求的记录
        $sql1="SELECT * FROM contact order by con_id LIMIT $offset,$page_size";
        $get = mysqli_query($con,$sql1);

            //将res结果集中查询结果取出一条
        while($row=mysqli_fetch_assoc($res)){
            echo"<tr><td>".$row["con_id"].
            "</td><td>".$row["name"].
            "</td><td>".$row["email"].
            "</td><td>".$row["phone"].
            "</td><td>".$row["content"].
            "</td><td><a href='mailto:someone@microsoft.com?subject=Hello%20again'>"."邮件"."</a></td>". 
            "</tr>";
        }
        ?>
    </tbody>
